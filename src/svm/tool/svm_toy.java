...
public class svm_toy extends Applet {
     ...
     */
    //public static final String dataFileDirPath = "D:\\work\\svmNew\\svm2.8\\data\\from2.6\\";
    //public static final String dataFileDirPath = "C:\\work\\dataAnalysis\\gasvm\\data\\";
    public static final String dataFileDirPath = gasvm.LearningProperties.svm_traindata_dir;

    static final String DEFAULT_PARAM="-t 2 -c 10 -g 1";
	int XLEN;
	int YLEN;
	...
	class point {
	    point(double x, double y, byte value, byte tag)
		{
			this.x = x;
			this.y = y;
			this.value = value;
	...
	public void init()
	{
        ...
        gridbag.setConstraints(button_next,c);
		c.weightx = 5;
		c.gridwidth = 5;
...
//        window_gc.drawString(String.valueOf(p.tag),(int)(p.x*XLEN),(int)(p.y*YLEN) );
	...
	void button_load_clicked()
	{
		...
		clear_all();
		try {
			BufferedReader fp = new BufferedReader(new FileReader(dataFileDirPath + filename));
			String line;
			while((line = fp.readLine()) != null)
			{
				...
				StringTokenizer st = new StringTokenizer(line," \t\n\r\f:");
				byte value = (byte)atoi(st.nextToken());
                //st.nextToken(); ÀÌÁÙ¸¸ »©°í ÇØº¸¼Ä~ sky1020
                byte tag = (byte)atoi(st.nextToken());
				st.nextToken();
				double x = atof(st.nextToken());
				st.nextToken();
				double y = atof(st.nextToken());
                System.out.println("[sky1020] x:" + x + ", y:" + y +  ", value:" + value);
				point_list.addElement(new point(x,y,value,tag));
    ...
    void button_next_clicked()
	{
		...
		try {
			...
			while((line = fp.readLine()) != null)
			{
                ...
                {
				    st.nextToken();
				    tmpx = atof(st.nextToken());
                }
                double x = tmpx;

                tmpvar=0;
                while((tmpvar++)!=(base_y-base_x))
                {
				    st.nextToken();
				    tmpy = atof(st.nextToken());
                }
                double y = tmpy;
                System.out.println("[sky1020] x:" + x + ", y:" + y +  ", value:" + value);
				point_list.addElement(new point(x,y,value,tag));
			}
            System.out.println("[Henry] x_basis:" + base_x + ", y_basis:" + base_y);
			fp.close();
		} catch (IOException e) { System.err.print(e); }
		draw_all_points();
	}
	...
	{
		new AppletFrame("svm.tool.svm_toy",new svm_toy(),gasvm.LearningProperties.svm_toy_size,gasvm.LearningProperties.svm_toy_size+50);
	}
}
